...
> **v0.1.0 实施决策（2026-08-22）：** `light` / `standard` 已闭合“Plan → 自动选队 → 批准 → Implement → Verify → Knowledge → Finish”和跨会话恢复。原设计中的高风险一次性授权体系不进入 v0.1.0；高风险 Task 实施由核心明确关闭失败，不存在调用方填写授权编号的绕过入口。
...
- 所有 `.ezagent/**` 状态变化只能通过打包 CLI；核心拒绝非法状态跳转、过期 revision、缺少审批和任何高风险 Task 实施命令。
...
| 高风险 | 数据删除、不可逆迁移、外部上传、安全边界变化 | v0.1.0 可分析和规划，但不进入实施；交由人工流程处理或拆分降险 |
...
不允许跳过 `approved` 直接进入正式实现。轻量工作项创建时由策略自动批准，但仍必须保留轻量 Spec 和验证记录。高风险 Task 在 v0.1.0 中不能进入 `implementing`。
...
4. **Approve**：按流程等级执行自动批准或标准 Spec 批准；高风险需求只能停留在分析/规划阶段。
