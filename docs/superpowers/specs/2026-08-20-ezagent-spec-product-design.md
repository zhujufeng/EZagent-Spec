...
公司大多数同事目前仍以 Vibe Coding 的方式使用代码 Agent：直接描述一个想法，让 Agent 立即修改代码，再通过多轮对话不断纠偏。这种方式在小型尝试中很直观，但进入真实项目后暴露出两个核心问题。

1. **开发效率较低。** 需求目标、范围、验收标准和实现约束没有先形成稳定 Spec，Agent 容易过早修改代码、理解偏移或只完成局部结果。同事需要反复解释、试错、回滚和重新验证，任务越复杂，返工成本越高。
2. **跨会话后 Agent 失忆。** 项目背景、已经确认的决策、当前进度、失败经验和后续任务主要留在单次聊天上下文中。新建会话、上下文压缩或切换同事后，Agent 无法可靠恢复工作现场，用户必须重新讲述，Agent 也可能重复工作或推翻之前的正确决定。
