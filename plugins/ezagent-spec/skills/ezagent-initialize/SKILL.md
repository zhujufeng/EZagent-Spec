...
description: 当用户明确要求在当前项目启用、初始化或安装 EZagent Work Harness 时，执行一次性环境检测、写入预览、确认和本地初始化。
---
...
6. 成功后说明以后只需用自然语言描述编码、分析、文档、策划或其他需求，相关请求会自动进入 Router；这些只是非穷尽示例，不设置固定人员或岗位。不要增加未在预览中出现的写入范围。
