...
- 没有剩余目标时正常结束，并说明已启用且已信任的 per-prompt Hook 会从下一条用户消息重新建立 Router 所有权，新写入的 `AGENTS.md` 会从下一次任务作为静态规则与 Hook 兜底自动生效；以后只需用自然语言描述编码、分析、文档、策划或其他需求。Codex 若提示 Hook 待审查，用户还需在 `/hooks` 中确认插件内 `UserPromptSubmit` Hook，未信任前不得声称每个 prompt 都会自动路由。
