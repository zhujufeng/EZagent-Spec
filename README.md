...
5. Evidence 缺失就把该 Slice 标记为 `revise`；完整才 `accepted`。
...
只有以下情况能证明收益时才使用 Specialist 或多 Agent：领域判断、上下文隔离、真正独立的并行 Slice、独立审查。每次委派必须绑定 Work Item、Work Spec、Slice、范围、交付物和 Evidence requirements，避免子 Agent 脱离主任务自由发挥。
...
- `.ezagent/**`
- `AGENTS.md#EZAGENT`
- `.codex/agents/ezagent-*.toml`（仅旧 v1 专家兼容流程需要）
