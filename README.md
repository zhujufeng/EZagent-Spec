...
先说结论：**0.5.0 当前仍是未发布候选版，暂不建议分发给普通同事。** 最新真实 Codex 宿主验收中 10 个场景通过 9 个，复杂 Specialist 规划仍出现过单回合超时；需要在修复后的最终提交上重新通过完整宿主验收和 Windows/macOS CI，才能改为“可分发”。本章保留为发布后的保姆级操作手册，避免届时让同事理解 CLI、JSON、Git、状态机或 Specialist ID。
...
支持。插件把每个路径作为独立 argv 参数处理，也为 PTY 无法关闭 stdin 的情况提供文件输入通道。需要临时文件时，它只会使用项目目录之外的操作系统临时目录，流程结束后自动清理，不会在你的项目里留下陌生 JSON 文件。仍有问题时，把完整报错和项目路径发给维护者，不要自行改 `.ezagent`。
