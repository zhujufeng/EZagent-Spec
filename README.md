...
先说结论：**0.5.0 已完成本地发布门和真实 Codex 宿主验收，达到正式发布候选标准。** 分发给普通同事时只使用 GitHub Releases 中带 `v0.5.0` 标签的正式版本，并确认该标签对应的 Windows/macOS CI 全绿；不要把未打标签的 `main` 分支压缩包当成正式版。本章就是给不懂 CLI、JSON、Git、状态机或 Specialist ID 的同事准备的保姆级操作手册。
