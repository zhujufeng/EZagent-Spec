...
先说结论：**0.5.1 已完成本地发布门、真实 Codex 宿主验收和 Windows/macOS CI，可以正式分发。** 分发给普通同事时只使用 GitHub Releases 中带 `v0.5.1` 标签的正式版本；不要把未打标签的 `main` 分支压缩包当成正式版。本章就是给不懂 CLI、JSON、Git、状态机或 Specialist ID 的同事准备的保姆级操作手册。
...
Planning-first 从正式版 `v0.5.1` 开始提供。已经安装 0.5.0 的同事需要升级插件、完全退出并重开宿主，再新建任务；旧任务不会在运行中自动加载新版规则。
