...
先说结论：**0.5.0 已完成本地发布门、真实 Codex 宿主验收和 Windows/macOS CI，可以正式分发。** 分发给普通同事时只使用 GitHub Releases 中带 `v0.5.0` 标签的正式版本；不要把未打标签的 `main` 分支压缩包当成正式版。本章就是给不懂 CLI、JSON、Git、状态机或 Specialist ID 的同事准备的保姆级操作手册。
...
截至当前正式版 `v0.5.0`，这项增强仍位于 `main` 的下一维护版开发内容中；要分发给同事使用，需等待带新版本标签的正式 Release。不要把现有 0.5.0 误认为已经包含该策略。
